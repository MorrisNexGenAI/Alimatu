...
export async function addGrade(gradeData: { student_id: number; subject_id: number;level_id:number;  period_id: number; score: number | null }[]) {
  ...
  const { subject_id, period_id, level_id } = gradeData[0]; // Assuming all entries have the same subject_id and period_id
    // You may need to dynamically fetch or pass the level_id
