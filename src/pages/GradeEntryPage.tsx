...
const GradeEntryPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchLevels = async () => {
      try {
        ...
        setLevels(data);
        console.log('Levels Data:', data); // Debug
      } catch (err) {
  ...
  useEffect(() => {
    const fetchPeriods = async () => {
      try {
        ...
        setPeriods(data);
        console.log('Periods Data:', data); // Debug
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        console.log('Students Data from API:', studentsData); // Debug
        console.log('Subjects Data from API:', subjectsData); // Debug
  ...
  useEffect(() => {
    console.log('Students State Updated:', students); // Debug
  }, [students]);

  useEffect(() => {
    console.log('Subjects State Updated:', subjects); // Debug
  }, [subjects]);
  ...
  const handleCheckExistingGrade = async () => {
    ...
    try {
      ...
      // Prepopulate grades with existing values
  ...
  const handleUpdateGrades = async () => {
    ...
    }).filter(grade => grade.id); // Only update grades with existing IDs
    try {
    ...
    <div className="p-4">
      ...
      <Select
        ...
        onChange={(e) => {
          ...
          console.log('Selected Level ID:', levelId); // Debug
        }}
        options={[
      ...
      {selectedLevelId && (
        <div className="flex gap-4 mb-4">
          <Select
            ...
            value={''} // No single student selection for bulk entry
            disabled={true}
  ...
  grades[student.id] !== undefined && grades[student.id] !== null
    ...
    : ''
          ...
          <p>No students available for this level.</p>
        )
      )}
    </div>
  );
